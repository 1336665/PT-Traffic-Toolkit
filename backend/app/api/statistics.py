...
async def get_overview(
    ...
    # Get real-time stats from downloaders
...
async def get_downloader_stats(
    ...
    for downloader in downloaders:
        ...
        # Get deleted stats for this downloader
        delete_result = await db.execute(
            select(
                func.count(DeleteRecord.id),
                func.coalesce(func.sum(DeleteRecord.size), 0)
            ).where(DeleteRecord.downloader_id == downloader.id)
        )
...
async def get_trend_data(
    ...
    # Initialize trend data dict for all days
    trend_dict = {}
    for i in range(days):
    ...
    delete_query = select(
        func.date(DeleteRecord.deleted_at).label('date'),
    ...
    ).group_by(func.date(DeleteRecord.deleted_at))
    ...
    speed_query = select(
        func.date(SpeedLimitRecord.created_at).label('date'),
    ...
    ).group_by(func.date(SpeedLimitRecord.created_at))
...
async def get_period_stats(
    ...
    for i in range(count):
        if period == "daily":
            ...
            period_name = start.strftime("%Y-%m-%d")
        elif period == "weekly":
            week_start = today - timedelta(days=today.weekday())
            start = week_start - timedelta(weeks=i)
            end = start + timedelta(weeks=1)
            period_name = f"Week {start.strftime('%Y-%m-%d')}"
        else:  # monthly
            ...
            period_name = start.strftime("%Y-%m")
