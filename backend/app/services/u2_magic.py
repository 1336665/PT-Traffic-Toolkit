...
class U2MagicService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.tid_add_time: Dict[str, float] = {} # 种子添加时间记录
    ...
    async def add_to_downloader(
        ...
        # 获取下载器
        result = await self.db.execute(
            select(Downloader).where(Downloader.id == config.downloader_id)
        )
        ...
        try:
            ...
            return torrent_hash is not None
        except Exception as e:
            logger.error(f"添加种子到下载器失败: {e}")
            return False
            ...
            try:
                ...
                if info:
                    ...
                    if torrent_data:
                        ...
                        # 添加到下载器
                        success = await self.add_to_downloader(torrent_data, config)
